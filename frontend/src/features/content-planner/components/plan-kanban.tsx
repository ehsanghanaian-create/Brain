        ...
        {cols.map((c) => (
          <div key={c.status} className='bg-muted/40 min-h-64 rounded-md p-1' onDragOver={(e) => e.preventDefault()} onDrop={() => drop(c.status)}>
            ...
            <div className='flex flex-col gap-1'>
              {c.items.map((p) => (
                <div key={p.id} draggable onDragStart={() => setDrag(p)} onClick={() => onOpen(p.id)} className='bg-card cursor-pointer rounded border p-1.5 text-xs shadow-sm hover:shadow' style={{ borderInlineStartWidth: 3, borderInlineStartColor: PRIORITY_COLOR[p.priority ?? ''] ?? '#e5e7eb' }}>
