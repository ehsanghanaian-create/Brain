...
def main() -> int:
    ...
    # ---- auto-sync scheduler plan (existing sync_runs + site_settings; no new tables)
    check("auto-sync plan (get)", "GET", api + f"/sites/{tmp}/auto-sync", 200, lambda r: r.json()["enabled"] is True and set(r.json()["sources"]) == {"wordpress", "gsc", "ga4"} and {"configured", "last_success", "next_at", "due"} <= set(r.json()["sources"]["gsc"]), headers=H)
    check("auto-sync toggle (put)", "PUT", api + f"/sites/{tmp}/auto-sync", 200, lambda r: r.json()["enabled"] is False and r.json()["interval_hours"] == 48, headers=H, json={"enabled": False, "interval_hours": 48})
    check("auto-sync bounds → 422", "PUT", api + f"/sites/{tmp}/auto-sync", 422, headers=H, json={"interval_hours": 999})
