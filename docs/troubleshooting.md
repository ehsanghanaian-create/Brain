...
| Symptom | Cause | Fix |
|---|---|---|
| `preflight.py` → `Python dependencies FAIL` | not running inside the venv | use `.venv\Scripts\python scripts\preflight.py` or `pip install -e .[dev]` |
...
| Duplicate nodes suspected | URL variants | check `src/normalizer/url.py` tests; builder validation prints `dup urls` |
