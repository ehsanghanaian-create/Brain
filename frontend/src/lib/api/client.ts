...
export const endpoints = {
  ...
  securityBlock: (id: string, ip: string, reason?: string) => api<{ success: boolean; ip?: string; status?: string; code?: string; message?: string }>(`/sites/${encodeURIComponent(id)}/security/block`, { method: 'POST', json: { ip, reason: reason || null } }),
  securityUnblock: (id: string, ip: string) => api<{ success: boolean; ip?: string; status?: string; code?: string; message?: string }>(`/sites/${encodeURIComponent(id)}/security/unblock`, { method: 'POST', json: { ip } }),
  securityResolveSite: (domain: string) => api<{ domain: string; site_id: string | null; configured: boolean }>(`/security/resolve-site?domain=${encodeURIComponent(domain)}`),
...
// ---- traffic intel: first-party visit tracking + Search Console join (migration 0018) -------------
